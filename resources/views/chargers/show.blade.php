...
<h1> Charging Station Charging History</h1>
<h2>{{$charger->name}}</h2>
</div>
 <div class="table-responsive-sm table-responsive-md table-hover">
    <table class="table">
      ...
      <tbody>
        @if (count($charger->car_charger) == 0)
        <h2 class='text-center text-primary'>This charger has not been visited by any cars.</h2>
        @endif
        @foreach ($charger->car_charger as $car_charger)
